...
static const  char *fragNV21 = GET_STR(
    precision mediump float;    //精度
    varying vec2 vTexCoord;     //顶点着色器传递坐标
    uniform sampler2D yTexture; //输入的材质(不透明度，单像素)
    uniform smapler2D uvTexture;
    ...
    void main(){
        ...
        yuv.b = texture2D(uvTexture,vTexCoord).a - 0.5;
        rbg = mat3(1.0,     1.0,    1.0,
                    0.0, -0.39465, 2.03211,
                    1.13983,-0.58060,0.0)*yuv;

        //输入像素颜色
        gl_FragColor = vec4(rbg,1.0);
    }
);
...
bool XShader::Init(XShaderType type)
{
    ...
    switch (type){
        ...
        case XSHADER_NV21:
        case XSHADER_NV12:
            glUniform1i(glGetUniformLocation(program,"uvTexture"),1);//对于纹理第二层
            break;
    }
    mux.unlock();
    XLOGE("初始化Shader成功！");
